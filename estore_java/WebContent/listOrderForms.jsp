...
<c:forEach items="${requestScope.orderFroms }" var="orderFrom" varStatus="status">           
		<tr>
			<td class=tablebody2 valign=middle align=center width="">${status.index }</td>
			<td class=tablebody1 valign=middle width="">&nbsp;&nbsp;<a href="user/orderinfo.info?orderid=1">${orderFrom.id }</a></td>
			<td class=tablebody2 valign=middle align=left width="">&nbsp;&nbsp;￥${orderFrom.cost }</td>
			...
			<input type="button" value="删除" onclick="javascript:window.location='user/removeOrder.action?orderid=1';">&nbsp;<input type="button" value="明细" onclick="javascript:window.location='listOrderLinesServlet?orderFormId=${orderFrom.id }'"><!--&nbsp;<input type="button" value="修改"/>--> </td>
		</tr>
</c:forEach>                     
                </table><br>
<!--文件尾开始-->
